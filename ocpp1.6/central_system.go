...
// drain — see SendRequestAsyncCtx), recovered handler panics (as
// *ocppj.HandlerPanicError — match with errors.As)).
...
//
// Every panic this central system recovers itself is reported, including those
// in inbound request, response and error handlers, the connect and disconnect
// handlers, an invalid-message hook registered on the underlying *ocppj.Server,
// and the canceled-request callback. So is every panic the DEFAULT dispatcher
// recovers internally. Panics recovered inside a CUSTOM ServerDispatcher are
// not — see the last paragraph.
//
...
//
// The chain order is observable: on a panic this central system reports on
// Errors() FIRST, then invokes any hook that was registered on the underlying
...
// this central system replaces the routing wrapper and silently disables the
...
func (cs *centralSystem) SetOnHandlerPanic(handler func(ocppj.HandlerPanic)) {
...
func (cs *centralSystem) handleIncomingRequest(chargePoint ChargePointConnection, request ocpp.Request, requestId string, action string) {
	...
	go func() {
		defer cs.server.RecoverPanicGoroutine(ocppj.RequestHandlerKind, chargePoint.ID(), action, requestId, true)
		switch action {
