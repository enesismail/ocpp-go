...
// RED-FIRST test suite - OCPP 1.6 central-system (server) facade. This is
// the SERVER mirror of the already-merged CLIENT-side E1c facade suite
// (ocpp1.6_test/context_send_test.go).
//
// RED-FIRST discipline: every test below references the PR-E2c surface
// exactly as the spec names it. Against today's codebase:
//   - CentralSystem.SendRequestAsyncCtx does not exist
//
...
// e2cWireUpServerAndClient stubs the mock ws server/client so a charge point
// "connects" to the central system, and every server-side Write is recorded
// on writtenC. Mirrors the manual-mock setup in
// ocpp1.6_test/e2_0_cross_delivery_test.go's
// TestE2_0_ServerCrossDeliveryRegression.
func e2cWireUpServerAndClient(suite *OcppV16TestSuite, channel MockWebSocket, writtenC chan []byte) {
...
// TestE2cCancelDispatchedServerRequest verifies that
// centralSystem.SendRequestAsyncCtx with a mid-flight-canceled context
// delivers exactly one callback, with an error matching both
// context.Canceled and ocppj.ErrRequestCanceled.
func (suite *OcppV16TestSuite) TestE2cCancelDispatchedServerRequest() {
	t := suite.T()
	wsID := "e2c-t1-cp"
	...
	// PR-E2c: CentralSystem.SendRequestAsyncCtx(ctx, clientId, request, callback).
	err = suite.centralSystem.SendRequestAsyncCtx(ctx, wsID,
...
// vacuous.
func (suite *OcppV16TestSuite) TestE2cCancelQueuedServerRequestNeverWritten() {
	t := suite.T()
	...
	// PR-E2c
	err = suite.centralSystem.SendRequestAsyncCtx(ctx2, wsID,
...
func (suite *OcppV16TestSuite) TestE2cSendRequestAsyncCtxNilBehavesAsBackground() {
	t := suite.T()
	wsID := "e2c-t7-cp"
	channel := NewMockWebSocket(wsID)
	...
	err = suite.centralSystem.SendRequestAsyncCtx(nil, wsID,
		core.NewChangeAvailabilityRequest(1, core.AvailabilityTypeOperative),
	...
	callResultJson := fmt.Sprintf(`[3,"%s",{"status":"%s"}]`, defaultMessageId, core.AvailabilityStatusAccepted)
	...
	case res := <-resultC:
		...
		require.NotNil(t, res.confirmation)
		_, ok := res.confirmation.(*core.ChangeAvailabilityConfirmation)
	...
	callResultJson2 := fmt.Sprintf(`[3,"%s",{"status":"%s"}]`, defaultMessageId, core.AvailabilityStatusAccepted)
