...
type csms struct {
	...
	// callbackQueue is a single queue shared across ALL connected charging
	// stations. SendRequestAsync registers a callback while holding the queue's
	// mutex across the enqueue into the dispatcher; if the shared dispatcher pump
	// is wedged on one stalled client's Write, that mutex is held facade-wide,
	// stalling every other client's response/error dequeue (the pump's own
	// canceled-request dequeue is deliberately off-pump — see handleCanceledRequest
	// — because on-pump it deadlocks rather than stalls). Server-side twin of the
	// client caveat in charging_station.go; pre-existing (unchanged by requestID
	// keying).
...
func (cs *csms) installDisconnectedHandler() {
	cs.server.SetDisconnectedClientHandler(func(chargingStation ws.Channel) {
		...
		for _, cb := range cs.callbackQueue.DrainAll(chargingStation.ID()) {
...
// Stop stops the CSMS. Cancellation of requests still held in the dispatcher's
// queues is scheduled during Stop and delivered exactly once with an error
// matching ocppj.ErrDispatcherStopped; if the stop drain races the peer's
// disconnect, the terminal sentinel may be ErrLocalTransport instead. Delivery
// is dispatched off the pump, so a callback may not yet have started when Stop
// returns. Cancellations for requests WITHOUT a registered callback are
// reported on Errors() best-effort (non-blocking, bounded buffer); Errors() is
// not a complete inventory of stop-drain cancellations.
func (cs *csms) Stop() {
	cs.server.Stop()
}
...
func (cs *csms) handleIncomingResponse(chargingStation ChargingStationConnection, response ocpp.Response, requestId string) {
	if callback, ok := cs.callbackQueue.Dequeue(chargingStation.ID(), requestId); ok {
		// Execute in separate goroutine, so the caller goroutine is available
		go func() {
...
func (cs *csms) handleIncomingError(chargingStation ChargingStationConnection, err *ocpp.Error, details interface{}) {
	if callback, ok := cs.callbackQueue.Dequeue(chargingStation.ID(), err.MessageId); ok {
		// Execute in separate goroutine, so the caller goroutine is available
		go func() {
