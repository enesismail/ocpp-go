...
//
// Test 8 — prefer-response fast-path (spec test 8, 2.0.1 facade).
//
// This file lives in `package ocpp2` (not `ocpp2_test`) so it can reach
// the unexported chargingStation.awaitCtxResult helper and the promoted
...
func TestE1cAwaitCtxResultPreLoadedResponseWinsOverCanceledCtx(t *testing.T) {
	cs := &chargingStation{}
	cs.storeStopC(make(chan struct{}, 1))
	...
	for i := 0; i < 100; i++ {
		...
		resp, err := cs.awaitCtxResult(ctx, "E1cTest", asyncResponseC, cs.loadStopC())
